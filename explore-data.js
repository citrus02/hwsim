...
function getMaterialChance(rarity, exploreRate) {
  ...
  // 探索度越高，获取几率越大
  const exploreBonus = exploreRate / 200; // 50%探索度时增加0.25的几率
  let chance = baseChances[rarity] + exploreBonus;
  
  return Math.min(chance, baseChances[rarity] * 2.5);
}
...
export function exploreMaterials(areaName, exploreRate) {
  ...
  // 随机决定获得数量 (根据稀有度决定)
  let maxQuantity = 1;
  if (rarityLevel === "common") maxQuantity = 3;
  else if (rarityLevel === "uncommon") maxQuantity = 2;
