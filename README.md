...
课堂叙事文本由 `course/classroom-render-utils.js` 中的 `_formatContext()` 渲染。换行应表达语义段落，而不是按屏幕宽度手动排版：

- 课程数据中显式写入的 `\n` 会作为段落换行保留。
- `「...」` 内的对白视为一个完整整体，即使包含 `。！？`，也不要在引号内部自动换行。
- `「变形术，」` 这类短对白片段属于所在句子的一部分，不应在其后换行。
- 只有当对白以 `。！？` 结束，且后面紧接新的叙述文字时，才可以在对白后换行。
- 不要在课程数据里为了适配某个屏幕宽度手动断行；段落内部的自然折行交给浏览器处理。
